...
io.on('connection', (socket) => {
  ...
  socket.on('find_game', () => {
    if (waiting.length > 0) {
      ...
      io.to(whiteSid).emit('available_cards', buildAvailableCardsFromGame(game));
    } else {
  ...
  socket.on('join_room', ({ roomId }) => {
    ...
    io.to(whiteSid).emit('available_cards', buildAvailableCardsFromGame(room.game));
  });
  ...
  socket.on('make_move', ({ roomId, from, to }) => {
    ...
    const playerColor =
      players.white === socket.id ? 'w' :
      players.black === socket.id ? 'b' : null;
